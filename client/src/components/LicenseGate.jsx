...
import { ShieldAlert, Key, Copy, Check, Lock, Cpu, Building2, User, RefreshCw, AlertCircle, FileText, CheckCircle2 } from 'lucide-react';
...
export default function LicenseGate({ hardwareId, errorReason, onActivated }) {
  ...
  const handleActivate = async (e) => {
    ...
    if (!licenseKeyInput.trim()) {
      setErrorMsg('لطفاً رشته کلید لایسنس را در کادر وارد نمایید یا فایل لایسنس را انتخاب کنید.');
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        setSuccessMsg('لایسنس با موفقیت فعال و تایید شد. در حال ورود به سامانه...');
        setTimeout(() => {
      ...
      <div className="max-w-xl w-full bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl p-6 sm:p-8 relative z-10">
        ...
        <div className="bg-slate-950/80 border border-slate-800 rounded-2xl p-4 mb-5">
          ...
          <p className="text-[11px] text-slate-500 mt-2 leading-relaxed">
            کد فوق را برای دریافت لایسنس دیجیتال به توسعه‌دهنده ارسال نمایید. لایسنس مختص به این سخت‌افزار قفل خواهد شد.
          </p>
        </div>

        {/* Developer Contact Card */}
        <div className="bg-gradient-to-r from-blue-950/30 to-indigo-950/30 border border-blue-900/30 rounded-2xl p-3.5 mb-5 flex items-center justify-between">
        ...
        <form onSubmit={handleActivate} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-black text-sm py-3 px-4 rounded-xl shadow-lg shadow-amber-500/20 transition active:scale-[0.99] disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
